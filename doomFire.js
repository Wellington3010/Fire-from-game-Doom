...
function start(){
    ...
    setInterval(createFirePropagation,50);
}
...
function createFirePropagation(){
    for(let column = 0; column < fireWidth; column ++){
        for(let row = 0; row < fireHeight; row++){
            const index2 = column + (fireWidth * row);
            UpdateFireIntensity(index2);
        }
...
if(belowPixel >= (fireWidth * fireHeight)){
 ...
 const currentFireIntensity = belowPixelFireIntensity - decay >= 0 ? belowPixelFireIntensity - decay : 0;

//  firePixelArray[currentPixel - decay] = currentFireIntensity;
...
function createFireSource(){
    for(let column = 0; column <= fireWidth; column++){
        const overflowPixelInder = fireWidth * fireHeight;
        let index = (overflowPixelInder - fireWidth) + column;

        firePixelArray[index] = 36;
    }
}
